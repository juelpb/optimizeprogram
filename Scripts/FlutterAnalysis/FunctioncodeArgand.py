...
def flutter_speed_DB(girder_height,tower_height,p_type,plot=None,p=None):
    ...
    def generate_modal_info_all_modes():
        ...
        #indexes = [1,2,3,4,5,24,27] # 40 first modes (multimodal)
    ...
    def eigen_solve(U,Omega,Modal,flag,threshold,maxit):  
        ...
        return Eigen_new,Miu,j,Omega, Omega_new,vector
    ...
    for i, U in enumerate(U_range):
        ...
        for flag, Omega in enumerate(Modal['Omega']):
            Eigen_new,Miu,j,Omega,Omega_new,vector   =  eigen_solve(U,Omega_lst[-1][flag],Modal,flag,threshold,maxit)
    ...
    return U_flutter, vector, flag


U_flutter, vector, flag = flutter_speed_DB(3.75, 220, p_type=2022,p=True,plot=False)
#%%
vector_cr = vector[flag]

labs = {1:'HS1',2:'VA1',3:'VS1',4:'HA1',5:'VS2',6:'TS1'}

import cmath
import seaborn as sns

fig, ax = plt.subplots(subplot_kw={'projection': 'polar'})
colors = sns.color_palette('tab20',n_colors=100)

for i in range(vector.shape[0]):
    vec_n = vector[:,i]
    
    max_idx = np.argmax(vec_n)
    print(max_idx)
    el = vec_n[max_idx]
    cord = cmath.polar(el)
    r = cord[0]
    ...
    if np.sin(theta) < 0:
        continue
    plt.plot([0,theta],[0,r],marker='o',color=colors[i],alpha=1,label=f"{i}")
    ...
    #     plt.plot([0,theta],[0,r],marker='o',color='red',alpha=0.5)
plt.legend()
plt.show()
