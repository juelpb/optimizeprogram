import DeflectionPrediction.DefPred as DP
import numpy as np
...
path = os.getcwd()
##########################
# !!! KEEP FOLLOWING LINES AT THEIR CURRENT LINE NUMBER !!!
tower_height=200
girder_def_mid=DP.Deflection(tower_height, 'girder_south')
girder_def_south=DP.Deflection(tower_height, 'girder_mid')
girder_def_north=DP.Deflection(tower_height, 'girder_north')
cable_def_mid=DP.Deflection(tower_height, 'cable_mid')
tower_def=DP.Deflection(tower_height, 'tower')
# LINE 19
# LINE 20
girder_area=1.2584885714285714
girder_I11=6.039942857142857
girder_I22=128.30242857142858
girder_It=20.398153846153846
# LINE 25
#
...
tower.z_crossbeam_south=np.array([60.0,tower_height-1])
tower.z_crossbeam_north=np.array([60.0,tower_height-1])
...
geo.dx_pullback_south=np.array(-1.19) #*np.nan, initial: -0.8
geo.dx_pullback_north=np.array(1.19) #*np.nan, initial: 0.8
...
geo.z_cable_top_south=np.array(tower_height+0.1)
geo.z_cable_top_north=np.array(tower_height+0.1)
...
geo.z_tower_top_south=np.array(tower_height)
geo.z_tower_top_north=np.array(tower_height)
